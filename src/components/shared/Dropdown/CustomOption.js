...
const getLabelSpan = ({ data }) => {
  ...
  return isHeader ? (
    <span class="ml-0 block truncate font-light uppercase leading-5 tracking-widest text-sm text-slate-gray">
      {label}
    </span>
  ) : (
    <div class="py-1 mb-1">
      <span class="ml-3 block truncate text-sm">{label}</span>
      {data.type === SEARCH_BAR_OPTIONS.PRODUCT && (
        <span class="ml-3 block truncate text-gray-500 uppercase font-extralight text-xs">
          {data.brandName}
        </span>
      )}
    </div>
